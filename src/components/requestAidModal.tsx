...
const RequestAidModal = (props: IRequestAidModalProps) => {
  const submitRequest = () => {
    ...
    if (category === '') {
      ...
      if (props.marker.length < 1) {
        alert('Shouldnt be possible, error here, length of props.marker has to be atleast 1')
        console.log(props)
        return
        ...
        alert('Shouldnt be possible, error here, length of props.marker can be max 2')
        return
      }
  ...
  return (
    <div
      className="bg-orange-50 flex flex-col justify-center items-center p-4 rounded-2xl"
      onClick={e => {
        e.stopPropagation()
      }}
    >
      <h1 className="text-2xl p-8 text-center font-roboto">What help do you need ?</h1>
      ...
      <div className="grid grid-cols-2 grid-rows-2 gap-x-2 gap-y-3 p-1">
        ...
        <button className="btn btn-primary text-white" onClick={() => setCategory('Essentials')}>
          Essentials
        </button>
        <button className="btn btn-primary text-white" onClick={() => setCategory('Accomodation')}>
          Accomodation
        </button>
        <button className="btn btn-primary text-white" onClick={() => setCategory('Others')}>
          Others
        </button>
      </div>
      <button
      ...
      <p className="underline text-accent text-center mt-10 mb-10 font-bold text-sm">
        Your current location along with information provided will be broadcasted for volunteering
        organisations to pick up
      </p>
    </div>
  )
}
